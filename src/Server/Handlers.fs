...
module Option =
  let insertToDb (db : DbFamilio) arg =
    task { let! inserted = db.InsertAsync(arg)
           return Some inserted }
  let taskNone<'a> : Threading.Tasks.Task<'a option> = task { return None }

module Account =
  let private getProps uri =
    AuthenticationProperties(RedirectUri = (if String.IsNullOrEmpty uri then
  ...
  let getApiToken (configuration : IConfigurationRoot) =
    task {
      ...
      let s =
        sprintf "grant_type=client_credentials&client_id=%s&client_secret=%s&audience=%s"
          configuration.["Auth0:ClientId"] configuration.["Auth0:ClientSecret"]
  ...
  let getAuth0Client (configuration : IConfigurationRoot) =
    task {
      let! token = task {
                     ...
                     | Error e ->
                       raise <| InvalidOperationException(e)
                       return ""
                     | Ok r -> return r.access_token
                   }
      return new ManagementApiClient(token, Uri(configuration.["Auth0:Identifier"]))
    }
  ...
  let currentUser (configuration : IConfigurationRoot) (next : HttpFunc) (ctx : HttpContext) =
    task {
      ...
      let id =
        ...
        | Some x -> x.Value
        | None -> ""
      let! resp = auth0Client.Users.GetAsync(id, "user_id,nickname,email", true)
      let user =
        { Id = resp.UserId |> Some
          Nickname = resp.NickName |> Some
          Email = resp.Email |> Some }
      logger.Log(LogLevel.Trace, EventId(0, "user"), sprintf "Got User: %A" user)
      return! json user next ctx
    }

  let callback next ctx = task { return! json null next ctx }
...
module Groups =
  ...
  let groupById configuration id next ctx =
    task {
      ...
      let tasksAndUsers =
        ...
        |> Array.map (fun t ->
             t,
             auth0Mgr.Users.GetAsync(t.Executor, "nickname", true)
             |> Async.AwaitTask
             |> Async.RunSynchronously
             |> (fun u -> u.NickName))
  ...
  let searchUserForGroup configuration ((groupId : int64), nick) next ctx =
    task {
      ...
      let req =
        GetUsersRequest
          (SearchEngine = "v3", IncludeFields = Nullable(true), Fields = "user_id,nickname,email",
      ...
      let users =
        ...
        |> Seq.filter
             (fun (u, _) ->
             not <| existingUsers.Any(fun urg -> urg.UserId = (u.Id |> Option.defaultValue "")))
  ...
  let addUserToGroup nick next (ctx : HttpContext) =
    task {
      ...
      let insert = Option.insertToDb db
      let! inserted = if groupExists then insert urg
                      else Option.taskNone
...
module Tasks =
  ...
  let closeTask (id : string) next ctx =
    task {
      ...
      use db = new DbFamilio()
      let update arg =
        task { let! updated = db.UpdateAsync(arg)
               return Some updated }
      ...
      let! _ = match task with
               | Some t -> update t
               | None -> Option.taskNone
      let resp =
        match task with
        | Some t -> json t next
        | None -> ((fun t ctx -> Response.badRequest ctx t) null)
      return! resp ctx
    }
