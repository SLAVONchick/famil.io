...
let postGroup (group : GroupDto) =
  ...
  let cmd =
    Cmd.ofPromise (res) [ Method HttpMethod.POST ] (Ok >> GroupUploaded) (Error >> GroupUploaded)
  cmd
...
let update state msg =
  ...
  | GroupUploaded res ->
    ...
    nextState, Cmd.none
  | OpenGroupCreationForm name -> GroupCreationFormOpened name, Cmd.none
  | Reset
  | CloseGroupCreationForm -> Initial, Cmd.none
  ...
  Media.media []
    [ Media.content []
        [ Content.content []
            [ div []
                [ strong []
                    [ a [ OnClick(fun _ -> navigateTo <| sprintf "#group/%d" g.Id) ] [ str g.Name ] ] ]

              div []
                [ small []
                    [ str <| sprintf "Created %s" (g.CreatedAt.ToString("dd.MM.yyyy hh:mm")) ] ] ] ] ]
...
let groupsView userId state (dispatch : Msg -> unit) navigateTo =
  ...
  | Loaded res ->
    ...
    | Ok gl ->
      div []
        ((gl |> List.map (groupToElement navigateTo))
